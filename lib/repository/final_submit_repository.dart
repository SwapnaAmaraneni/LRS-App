...
class VillagewiseClusterInspectionFinalSubmitRepository {
  ...
  Future<FinalSubmitResponse?> bulkSubmitApi(
      BuildContext context, BulkUpdatePayload payload) async {
    try {
      final payloadJson = payload.toJson();
      final strJson = jsonEncode(payloadJson);
      ...
      final token = CancelToken();
      final response = await _baseApiClinet.postCall(
          ApiConstants.bulkUpdateEndpoint, payload.toJson(), token);
      return FinalSubmitResponse.fromJson(response);
    } catch (e) {
      return Future.error(e);
    }
