...
class ProhibitedChecklist extends StatefulWidget {
  const ProhibitedChecklist({super.key});

  @override
  State<ProhibitedChecklist> createState() => _ProhibitedChecklistState();
}

class _ProhibitedChecklistState extends State<ProhibitedChecklist> {
  final urlPattern = r'^(http|https):\/\/([\w.]+\/?)\S*';

  @override
  Widget build(BuildContext context) {
    final checkDetailsProvider =
        Provider.of<ProhibitedCheckDetailsViewModel>(context);
    return Stack(
      children: [
        Scaffold(
          appBar: const AppBarReusable(
            title: "Prohibited Check Details",
          ),
          body: Stack(
            children: [
              Container(
                          ...
                          ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              if (quesResponse.selectedAnswer == "Y" &&
                                  ...
                                  checkDetailsProvider.remarksControllers.isNotEmpty) {
                                checkDetailsProvider.remarksControllers[index].text =
                                  ...
                                  if (quesResponse.selectedAnswer == "Y" &&
                                    ...
                                    SizedBox(
                                      width: MediaQuery.of(context).size.width *
                                          0.9,
                                      child: TextField(
                                        controller: checkDetailsProvider.remarksControllers[index],
                                        maxLines: null,
                                        maxLength: 300,
                                        inputFormatters: [
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      final checkDetailsProvider =
          Provider.of<ProhibitedCheckDetailsViewModel>(context, listen: false);

      // Fetch questions only on first visit; skip on back-navigation
      await checkDetailsProvider.checkDetailsViewModel(context);

      // Restore saved checklist answers from SharedPreferences only once
      if (checkDetailsProvider.checkList.isEmpty) {
        ...
        final List<dynamic> checkListJson =
            (checkListString != null && checkListString != "")
                ...
                : [];
        checkDetailsProvider.checkList.addAll(checkListJson
            .map((checkListMap) => ListSaveData.fromJson(checkListMap))
            .toList());
      }

      // Create remarks controllers only once
      if (checkDetailsProvider.remarksControllers.isEmpty &&
          checkDetailsProvider.applications.isNotEmpty) {
        checkDetailsProvider.remarksControllers.addAll(List.generate(
          checkDetailsProvider.applications.length,
          (index) => TextEditingController(),
        ));
        if (!kReleaseMode) {
          debugPrint(
              "remarks length::  ${checkDetailsProvider.remarksControllers.length}");
        }
      }

      // Restore responses from checkList only if not already populated
      if (checkDetailsProvider.responses.isEmpty) {
        for (var element in checkDetailsProvider.checkList) {
