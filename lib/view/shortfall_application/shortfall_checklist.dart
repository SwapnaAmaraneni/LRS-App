...
class ShortfallChecklist extends StatefulWidget {
  const ShortfallChecklist({super.key});

  @override
  State<ShortfallChecklist> createState() => _ShortfallChecklistState();
}

class _ShortfallChecklistState extends State<ShortfallChecklist> {
  final urlPattern = r'^(http|https):\/\/([\w.]+\/?)\S*';

  @override
  Widget build(BuildContext context) {
    final checkDetailsProvider =
        Provider.of<ShortfallCheckDetailsViewModel>(context);
    return Stack(
      children: [
        Scaffold(
          appBar: const AppBarReusable(
            title: "Shortfall Check Details",
          ),
          body: Stack(
            children: [
              Container(
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      final checkDetailsProvider =
          Provider.of<ShortfallCheckDetailsViewModel>(context, listen: false);
      if (!checkDetailsProvider.isInitialized) {
        ...
        if (!kReleaseMode) {
          debugPrint("remarks length::  ${checkDetailsProvider.remarksControllers.length}");
        }
        for (var element in checkDetailsProvider.checkList) {
          if (element.sUBCHECKID != null && element.sTATUSID == "Y") {
            if (!mounted) return;
            checkDetailsProvider.getDropdownValues(
        ...
        checkDetailsProvider.isInitialized = true;
      }
    });
  }
}
