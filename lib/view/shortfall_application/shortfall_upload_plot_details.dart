...
class ShortfallUploadPlotDetails extends StatefulWidget {
  const ShortfallUploadPlotDetails({super.key});

  @override
  State<ShortfallUploadPlotDetails> createState() =>
      _ShortfallUploadPlotDetailsState();
}

class _ShortfallUploadPlotDetailsState
    extends State<ShortfallUploadPlotDetails> {
  List gridsData = [
    "plotImg1".tr(),
    "plotImg2".tr(),
    "plotImg3".tr(),
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      final uploadPlotDetailsProvider =
          Provider.of<ShortfallUploadPlotDetailsViewModel>(context,
              listen: false);
      ...
      if (locEnabled) {
        ...
        if (!uploadPlotDetailsProvider.isInitialized) {
          ...
          uploadPlotDetailsProvider.ecSelectedDoc = prefs.getString(SharedPrefConstants.ecSelectedDocKey);
          uploadPlotDetailsProvider.ownershipSelectedDoc =
              prefs.getString(SharedPrefConstants.ownershipSelectedDocKey);
          uploadPlotDetailsProvider.imagesString[0] = prefs.getString(SharedPrefConstants.plot1Img) ?? "";
          uploadPlotDetailsProvider.imagesString[1] = prefs.getString(SharedPrefConstants.plot2Img) ?? "";
          uploadPlotDetailsProvider.imagesString[2] = prefs.getString(SharedPrefConstants.plot3Img) ?? "";
          ...
          if ((uploadPlotDetailsProvider.layoutSelectedDoc != null && uploadPlotDetailsProvider.layoutSelectedDoc != "") ||
          ...
          if ((uploadPlotDetailsProvider.ecSelectedDoc != null && uploadPlotDetailsProvider.ecSelectedDoc != "") ||
          ...
          if ((uploadPlotDetailsProvider.ownershipSelectedDoc != null && uploadPlotDetailsProvider.ownershipSelectedDoc != "") ||
          ...
          uploadPlotDetailsProvider.isInitialized = true;
        }
      ...
      children: [
        Scaffold(
          appBar: AppBarReusable(
            title: "Shortfall Plot Details Upload",
            leading: IconButton(
                onPressed: () {
                  ...
                  children: [
                      ...
                      child: Column(
                        children: [
                          QuestionTile(
                            ...
                            selectedAnswer: uploadPlotDetailsProvider.layoutSelectedAnswer ?? "",
                            onChanged: (answer) {
                              setState(() {
                                uploadPlotDetailsProvider.layoutSelectedAnswer = answer;
                                // SharedPrefConstants.layoutDocumentradioVal = layoutSelectedAnswer;
                              });
                            },
                          ),
                          uploadPlotDetailsProvider.layoutSelectedAnswer?.toLowerCase() == "y" ||
                                  uploadPlotDetailsProvider.layoutSelectedAnswer?.toLowerCase() == "yes"
                              ? Column(
                                  children: [
                                    Stack(
                                      children: [
                                          ...
                                          children: [
                                            FilePickerComponent(
                                              ...
                                              callbackValue: (File file) {
                                                uploadPlotDetailsProvider.layoutSelectedDoc = file.path;
                                              },
                                            ),
                                            const Text(
                                              "*",
                      ...
                      child: Column(
                        children: [
                          QuestionTile(
                            ...
                            selectedAnswer: uploadPlotDetailsProvider.ownershipDocSelectedAnswer ?? "",
                            onChanged: (answer) {
                              setState(() {
                                uploadPlotDetailsProvider.ownershipDocSelectedAnswer = answer;
                              });
                            },
                          ),
                          uploadPlotDetailsProvider.ownershipDocSelectedAnswer?.toLowerCase() == "y" ||
                                  uploadPlotDetailsProvider.ownershipDocSelectedAnswer?.toLowerCase() ==
                                      "yes"
                              ? Column(
                                  children: [
                                    Row(
                                      ...
                                      children: [
                                        FilePickerComponent(
                                          ...
                                          filepath: getNormalizedPath(
                                              uploadPlotDetailsProvider.ownershipSelectedDoc),
                                          callbackValue: (File file) {
                                            uploadPlotDetailsProvider.ownershipSelectedDoc = file.path;
                                          },
                                        ),
                                        const Text(
                                          "*",
                      ...
                      child: Column(
                        children: [
                          QuestionTile(
                            ...
                            selectedAnswer: uploadPlotDetailsProvider.ecDocSelectedAnswer ?? "",
                            onChanged: (answer) {
                              setState(() {
                                uploadPlotDetailsProvider.ecDocSelectedAnswer = answer;
                              });
                            },
                          ),
                          uploadPlotDetailsProvider.ecDocSelectedAnswer?.toLowerCase() == "y" ||
                                  uploadPlotDetailsProvider.ecDocSelectedAnswer?.toLowerCase() == "yes"
                              ? Column(
                                  children: [
                                    Row(
                                      mainAxisAlignment:
                                      ...
                                      children: [
                                        FilePickerComponent(
                                          filepath:
                                              getNormalizedPath(uploadPlotDetailsProvider.ecSelectedDoc),
                                          callbackValue: (File file) {
                                            uploadPlotDetailsProvider.ecSelectedDoc = file.path;
                                          },
                                        ),
                                        const Text(
                                          "*",
                    ...
                    GridView.count(
                      ...
                      children: List.generate(gridsData.length, (index) {
                        return Center(
                          child: Card(
                            child: gridItem(gridsData[index], index, uploadPlotDetailsProvider),
                          ),
                        );
                      }),
                    ),
                    ...
                    ReusableButton(
                      ...
                      onPressed: () {
                        uploadPlotDetailsProvider.validationsForPlotDetails(
                            context: context,
                            layoutSelectedAnswer: uploadPlotDetailsProvider.layoutSelectedAnswer,
                            captureLocScreenshot: uploadPlotDetailsProvider.captureLocScreenshot,
                            ecDocSelectedAnswer: uploadPlotDetailsProvider.ecDocSelectedAnswer,
                            ecSelectedDoc: uploadPlotDetailsProvider.ecSelectedDoc,
                            images: uploadPlotDetailsProvider.imagesString,
                            layoutSelectedDoc: uploadPlotDetailsProvider.layoutSelectedDoc,
                            ownershipDocSelectedAnswer:
                                uploadPlotDetailsProvider.ownershipDocSelectedAnswer,
                            ownershipSelectedDoc: uploadPlotDetailsProvider.ownershipSelectedDoc,
  ...
  Widget gridItem(
    ...
    String label,
    int index,
    ShortfallUploadPlotDetailsViewModel provider,
  ) {
    if (!kReleaseMode) debugPrint("$index Image::: ${provider.imagesString[index]}");
    return ImageCaptureComponent(
        networkImg: provider.imagesString[index],
        callbackValue: (XFile file) {
          provider.imagesString[index] = file.path;
        },
        label: label);
  }
}
