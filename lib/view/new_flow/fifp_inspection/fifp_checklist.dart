...
class FifpChecklist extends StatefulWidget {
  const FifpChecklist({super.key});

  @override
  State<FifpChecklist> createState() => _FifpChecklistState();
}

class _FifpChecklistState extends State<FifpChecklist> {
  @override
  Widget build(BuildContext context) {
    final checkDetailsProvider = Provider.of<FifpCheckListViewModel>(context);
    return Stack(
      children: [
        Scaffold(
          appBar: const AppBarReusable(
            title: "Fee Paid Check Details",
          ),
          body: Stack(
            children: [
              Container(
                          ...
                          ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              if (quesResponse.selectedAnswer == "Y" &&
                                ...
                                checkDetailsProvider.remarksControllers[index]
                                    .text = quesResponse.remarks ?? "";
                              }
                                  ...
                                  if (quesResponse.selectedAnswer == "Y" &&
                                    ...
                                    SizedBox(
                                      width: MediaQuery.of(context).size.width *
                                          0.9,
                                      child: TextField(
                                        controller: checkDetailsProvider
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      final checkDetailsProvider =
          Provider.of<FifpCheckListViewModel>(context, listen: false);
      if (checkDetailsProvider.applications.isEmpty &&
          checkDetailsProvider.remarksControllers.isEmpty) {
        ...
        final List<dynamic> checkListJson =
            (checkListString != null && checkListString != "")
                ...
                : [];
        checkDetailsProvider.checkList.addAll(
          checkListJson
              .map(
                (checkListMap) => ListSaveData.fromJson(checkListMap),
              )
              .toList(),
        );
