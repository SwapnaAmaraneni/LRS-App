...
class FifpUploadPlotDetails extends StatefulWidget {
  const FifpUploadPlotDetails({super.key});

  @override
  State<FifpUploadPlotDetails> createState() => _FifpUploadPlotDetailsState();
}

class _FifpUploadPlotDetailsState extends State<FifpUploadPlotDetails> {
  List gridsData = [
    "plotImg1".tr(),
    "plotImg2".tr(),
    "plotImg3".tr(),
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      final uploadPlotDetailsProvider =
          Provider.of<FifpUploadPlotDetailsViewModel>(context, listen: false);
      if (uploadPlotDetailsProvider.imagesString.isEmpty) {
        uploadPlotDetailsProvider.imagesString =
      ...
      if (locEnabled) {
        ...
        bool isAnyDataPresent = uploadPlotDetailsProvider.imagesString
                .any((element) => element.isNotEmpty) ||
            (uploadPlotDetailsProvider.layoutSelectedDoc?.isNotEmpty ??
                false) ||
            (uploadPlotDetailsProvider.ecSelectedDoc?.isNotEmpty ?? false) ||
            (uploadPlotDetailsProvider.ownershipSelectedDoc?.isNotEmpty ??
                false) ||
            (uploadPlotDetailsProvider.layoutSelectedAnswer?.isNotEmpty ??
                false) ||
            (uploadPlotDetailsProvider.ecDocSelectedAnswer?.isNotEmpty ??
                false) ||
            (uploadPlotDetailsProvider.ownershipDocSelectedAnswer?.isNotEmpty ??
                false);
  ...
  Widget build(BuildContext context) {
    final uploadPlotDetailsProvider =
        Provider.of<FifpUploadPlotDetailsViewModel>(context);
    final imgLoader = Provider.of<ImagePickerLoader>(context);
    return Stack(
      children: [
        Scaffold(
          appBar: AppBarReusable(
            title: "Fee Paid Upload Plot Details",
            leading: IconButton(
                onPressed: () {
                          ...
                          QuestionTile(
                            ...
                            onChanged: (answer) {
                              setState(() {
                                uploadPlotDetailsProvider.ecDocSelectedAnswer =
                                    answer;
                                uploadPlotDetailsProvider.ecSelectedDoc = null;
                              });
                            },
                          ),
                          uploadPlotDetailsProvider.ecDocSelectedAnswer
