...
class Phase2ShortfallAddUnsoldPlotDetails extends StatefulWidget {
  const Phase2ShortfallAddUnsoldPlotDetails({super.key});

  @override
  State<Phase2ShortfallAddUnsoldPlotDetails> createState() =>
      _Phase2ShortfallAddUnsoldPlotDetailsState();
}

class _Phase2ShortfallAddUnsoldPlotDetailsState
    extends State<Phase2ShortfallAddUnsoldPlotDetails> {
  int unsoldPlotsCount = 0;
  @override
  void initState() {
    super.initState();
    final phase2ShortfallLayoutsProvider =
        Provider.of<Phase2ShortfallLayoutApplicationDetailsViewModel>(context,
            listen: false);
    phase2ShortfallLayoutsProvider.plotDetailsList.clear();
    WidgetsBinding.instance.addPostFrameCallback(
      (timeStamp) async {
        SharedPreferences prefs = await SharedPreferences.getInstance();
        final list = prefs.getString(SharedPrefConstants.unsoldPlotListKey);
        unsoldPlotsCount = int.tryParse(
                (prefs.getString(SharedPrefConstants.unsoldPlotsKey) ?? "0")) ??
            0;
        ...
        phase2ShortfallLayoutsProvider.plotDetailsList
            .addAll(finalUnsoldPlotsList);
        setState(() {});
      },
    );
    AppLogger().logDebug(
        "initstate length ::: ${phase2ShortfallLayoutsProvider.plotDetailsList.length}");
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (!kReleaseMode) {
      debugPrint(
          "list:: ${phase2ShortfallLayoutsProvider.plotDetailsList.length}");
    }
    return PopScope(
      canPop: false,
      onPopInvokedWithResult: (didPop, result) {
        if (!didPop) {
          WarningCustomCupertinoAlertTwoButtons().showAlert(
            context,
            message:
                "Data of the Application ID : ${phase2ShortfallLayoutsProvider.clusterApplDetails[0].aPPLICATIONID} will be lost. Do you want to continue? ",
            onPressedOk: () {
              phase2ShortfallLayoutsProvider.plotDetailsList.clear();
              Navigator.popUntil(
                context,
      ...
      child: Scaffold(
        appBar: const AppBarReusable(
          title: "Phase-2 Shortfall Unsold Plots List",
        ),
        body: SafeArea(
          child: Stack(
            ...
            children: [
              Container(
                ...
                child: Column(
                  children: [
                    getTitleCard(
                        "Total Unsold Plots Count: $unsoldPlotsCount", context),
                    (!(phase2ShortfallLayoutsProvider.plotDetailsList.length ==
                                unsoldPlotsCount ||
                            phase2ShortfallLayoutsProvider
                        ...
                        ? Card(
                            child: Column(
                              ...
                              children: [
                                ...
                                AppInputTextfield(
                                  hintText: "Plot No",
                                  nameController: phase2ShortfallLayoutsProvider
                                      .plotNoController,
                                ),
                                AppInputTextfield(
                    ...
                    unsoldPlotsCount != 0
                        ? Expanded(
                            child: ListView.builder(
                              itemCount: phase2ShortfallLayoutsProvider
                                  .plotDetailsList.length,
                              itemBuilder: (context, index) {
                                ...
                                final details = phase2ShortfallLayoutsProvider
                    ...
                    Align(
                      ...
                      child: ReusableButton(
                        ...
                        onPressed: () {
                          if (!kReleaseMode) {
                            debugPrint(
                                "length list ::: ${phase2ShortfallLayoutsProvider.plotDetailsList.length}");
                          }
                          if (phase2ShortfallLayoutsProvider
                                  .plotDetailsList.length ==
                              unsoldPlotsCount) {
                            ...
                            ErrorCustomCupertinoAlert().showAlert(context,
                                message:
                                    "Unsold Plots count and added details count does not match, Please check.");
                          }
                        },
                      ),
                    ),
