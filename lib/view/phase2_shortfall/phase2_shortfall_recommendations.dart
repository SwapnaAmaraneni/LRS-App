...
class Phase2ShortfallRecommendations extends StatefulWidget {
  const Phase2ShortfallRecommendations({super.key});

  @override
  State<Phase2ShortfallRecommendations> createState() =>
      _Phase2ShortfallRecommendationsState();
}

class _Phase2ShortfallRecommendationsState
    extends State<Phase2ShortfallRecommendations> {
  TextEditingController remarksController = TextEditingController();
  @override
  Widget build(BuildContext context) {
    if (!kReleaseMode) debugPrint("UserType:: ${AppConstants.userType}");
    final recommendationsProvider =
        Provider.of<Phase2ShortfallRecommendationsModel>(context);
    return Stack(
      children: [
        Scaffold(
          appBar: const AppBarReusable(
            title: "Phase-2 Shortfall Recommendations",
          ),
          body: SafeArea(
            child: Container(
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      if (AppConstants.isSavedApplication == "yes") {
        ...
        final recommendation =
          ...
          orElse: () =>
              recommendationsProvider.recommendationListMasterPlans[0],
        );
        recommendationsProvider.setDropdownValue(recommendation);
        remarksController.text = savedRemarks ?? "";
      }
    });
  }
}
