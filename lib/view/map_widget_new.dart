...
class MapWidgetNew extends StatefulWidget {
  const MapWidgetNew({
    super.key,
  ...
  @override
  State<MapWidgetNew> createState() => _MapWidgetState();
}

class _MapWidgetState extends State<MapWidgetNew> {
              ...
              child: Row(
                children: [
                  PolygonBottomsheetColumnComponent(
                    ...
                    onTap: (timer == null || !timer!.isActive)
                        ? () async {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text("Location Capturing Started"),
                              ),
                            );
                            addmarkers(markerTimer: timer);
                          }
                        : null,
                  ),
                  PolygonBottomsheetColumnComponent(
                    title: "Stop",
                    assetPath: AppAssets.stop,
                    onTap: () async {
                      if (timer != null) {
                        stopLocationUpdates();
                      }
                    ...
                    onTap: () async {
                      if (timer == null) {
                        ...
                        WarningCustomCupertinoAlert().showAlert(context,
                            message: "Please Stop Location Capturing to view",
                            onPressed: () {
                  ...
                  PolygonBottomsheetColumnComponent(
                    ...
                    onTap: () async {
                      if (timer == null &&
                          captureGeoCoordinatesProvider.markers.isNotEmpty) {
                  ...
                  PolygonBottomsheetColumnComponent(
                      ...
                      onTap: () async {
                        ...
                        await captureGeoCoordinatesProvider.onSave(context);
  ...
  bool isMapCentered(LatLng userLocation, LatLng mapCenter,
  ...
  List<LatLng> grahamScan(List<LatLng> points) {
    ...
    captureGeoCoordinatesProvider.addSortedLatLng(sortedPoints);
...
// Cross product of vectors (p1->p2) and (p1->p3)
  ...
  void stopLocationUpdates() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text("Location Capturing Stopped"),
      ),
    );
  }

  addmarkers({Timer? markerTimer}) {
    final captureGeoCoordinatesProvider =
        Provider.of<CaptureGeoCoordinatesViewModelNew>(context, listen: false);
    timer = markerTimer;
        ...
        try {
          await Geolocator.getCurrentPosition(
            ...
            (currentPos) async {
              ...
              if (!mounted) return;
              await captureGeoCoordinatesProvider.onAdd(context, currentLatLng);
            },
          );
  ...
  showAlert() {
    final captureGeoCoordinatesProvider =
        Provider.of<CaptureGeoCoordinatesViewModelNew>(context, listen: false);
    if (!captureGeoCoordinatesProvider.isPolygonSaved &&
        (captureGeoCoordinatesProvider.markers.isNotEmpty ||
            captureGeoCoordinatesProvider.latlongsList.isNotEmpty)) {
      WarningCustomCupertinoAlertTwoButtons().showAlert(
        context,
        message: "Data will be lost, Do you want to continue?",
        onPressedOk: () {
          captureGeoCoordinatesProvider.onClear(context);
          stopLocationUpdates();
          Navigator.popUntil(
