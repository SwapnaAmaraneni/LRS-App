...
class IGRSUploadPlotDetails extends StatefulWidget {
  const IGRSUploadPlotDetails({super.key});

  @override
  State<IGRSUploadPlotDetails> createState() => _IGRSUploadPlotDetailsState();
}

class _IGRSUploadPlotDetailsState extends State<IGRSUploadPlotDetails> {
  List gridsData = [
    "plotImg1".tr(),
    "plotImg2".tr(),
    "plotImg3".tr(),
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      final uploadPlotDetailsProvider =
          Provider.of<IGRSUploadPlotDetailsViewModel>(context, listen: false);
  ...
  Widget build(BuildContext context) {
    final uploadPlotDetailsProvider =
        Provider.of<IGRSUploadPlotDetailsViewModel>(context);
    final imgLoader = Provider.of<ImagePickerLoader>(context);
    return Stack(
      children: [
        Scaffold(
          appBar: AppBarReusable(
            title: "IGRS Upload Plot Details",
            leading: IconButton(
                onPressed: () {
                                  ...
                                  children: [
                                    Row(
                                      ...
                                      children: [
                                        FilePickerComponent(
                                          filepath:
                                              getNormalizedPath(ecSelectedDoc),
                                          callbackValue: (File file) {
