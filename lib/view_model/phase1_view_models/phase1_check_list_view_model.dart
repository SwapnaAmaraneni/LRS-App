...
class Phase1CheckDetailsViewModel with ChangeNotifier {
  ...
  Future<void> submit(BuildContext context) async {
    ...
    userType.toString().toLowerCase() == "tp"
        ...
        : Navigator.pushNamed(context, AppRoutes.phase1Recommendations);
    // }
  }

  bool validateQuestions(BuildContext context) {
    List<String> unansweredQuestions = [];
    for (var question in applications) {
      if (!responses.any((element) => element.sno == question.sNO)) {
        unansweredQuestions.add(question.sNO!);
      }
    }
    if (unansweredQuestions.isEmpty) {
      return true;
    } else {
      // Show a pop-up for each unanswered question
      for (var sno in unansweredQuestions) {
        ValidationIoSAlert().showAlert(
          context,
          description: "Please select an answer for question number $sno.",
        );
        break;
      }
      return false;
