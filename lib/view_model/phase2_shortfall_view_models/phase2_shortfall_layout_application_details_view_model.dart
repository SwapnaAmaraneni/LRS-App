...
import 'package:lrsofficer/repository/phase2_shortfall_repo/phase2_shortfall_applications_repo.dart';
...
class Phase2ShortfallLayoutApplicationDetailsViewModel with ChangeNotifier {
    ...
    try {
      if (await internetCheck()) {
        ...
        SavedApplicationDetailsResponse? response =
            await Phase2ShortfallApplDetailsRepository()
                .phase2ShortfallApplDetailsRepo(context, request);
  ...
  Future<void> navigateToUploadDocsScreen(
    ...
    if (selectedUnsoldPlotDetails == '' || selectedUnsoldPlotDetails == null) {
...
/*       await prefs.setString(SharedPrefConstants.plotNoKey, plotNo);
      ...
      if (AppConstants.isLayoutPlot == "L" &&
        ...
        Navigator.pushNamed(context, AppRoutes.phase2ShortfallAddUnsoldPlots);
        ...
        Navigator.pushNamed(
            context, AppRoutes.phase2ShortfallUploadPlotDetails);
      }
    }
  }
  ...
  bool validateNewMobileNo(
    ...
    if (mobileNo.isEmpty) {
      ErrorCustomCupertinoAlert().showAlert(
        context,
        message: "Enter phase2 mobile number",
        onPressed: () async {
          Navigator.of(context).pop();
        },
      );
      ...
      ErrorCustomCupertinoAlert().showAlert(
        context,
        message: "Enter phase2 Valid mobile number",
        onPressed: () async {
          Navigator.of(context).pop();
        },
      );
  ...
  Future<void> validateOTp(BuildContext context, BuildContext dialogContext,
      String otpText, String phase2MobileNo) async {
    ...
    if (otpText.isEmpty || otpText.length < 4) {
      ...
      SuccessCustomCupertinoAlert().showAlert(
          ...
          onPressed: () async {
            ...
            await updateMobileProvider.updatePhase2ShortfallMobileApi(
              context,
  ...
  Future<void> submitAndSave(BuildContext context) async {
    ...
    if (!context.mounted) return;
    Navigator.pushNamed(context, AppRoutes.phase2ShortfallUploadPlotDetails);
  }
}
