...
import 'package:lrsofficer/repository/prohibited_applications_repo.dart';
...
class ProhibitedApplicationDetailsViewModel extends ChangeNotifier {
  ...
  // Editable controllers – kept in ViewModel so values survive navigation
  ...
  Future<void> getClusterApplDetails(BuildContext context) async {
    try {
      ...
      if (clusterApplDetails.isNotEmpty) return;
      if (await internetCheck()) {
        ...
        ClusterApplicationDetailsResponse? response =
            await ProhibitedApplDetailsRepository()
                .prohibitedApplDetailsRepo(context, request);
  ...
  Future<void> getListOfMasterPlansZDPDetails(BuildContext context) async {
    try {
      // Skip re-fetching if data is already loaded
      if (listMasterPlansZDP.isNotEmpty) return;
      if (await internetCheck()) {
        setLoaderVisibleStatus(true);
        ...
        if (response != null && response.statusCode == ApiErrorCodes.success) {
          ...
          if (applicationsCount.isNotEmpty) {
            ...
            // Only set default ZDP if no selection has been made yet
  ...
  Future<void> navigateToUploadDocsScreen(
    ...
    if (isSroEdited == true && sroCode.length != 4) {
      ...
      if (!context.mounted) return;
      Navigator.pushNamed(context, AppRoutes.prohibitedUploadPlotDetails);
    }
  }
    ...
    if (otpText.isEmpty || otpText.length < 4) {
      ...
      SuccessCustomCupertinoAlert().showAlert(
          ...
          onPressed: () async {
            ...
            await updateMobileProvider.updateMobileProhibitedAppApi(
              context,
  ...
  Future<void> revenueNavigation({
    ...
    if (!context.mounted) return;
    Navigator.pushNamed(context, AppRoutes.prohibitedChecklist);
  }
}
