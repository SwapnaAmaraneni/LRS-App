...
import 'package:lrsofficer/models/bulk_update_payload.dart';
...
class RecommendationsViewModel extends ChangeNotifier {
  ...
  Future<void> getRecommendationsDetails(BuildContext context) async {
    try {
      // Skip re-fetching if data is already loaded
      if (recommendationListMasterPlans.isNotEmpty) return;
    ...
    try {
      ...
      for (CHECKLIST item in checkList) {
        ...
        if (item.docment != null) {
          item.docment = await processFileOrUrl(item.docment ?? "");
        }
      }
...
/*       String? latitude = prefs.getString(SharedPrefConstants.latitudeKey);
      String? longitude = prefs.getString(SharedPrefConstants.longitudeKey); */
              ...
              String? sroCode = prefs.getString(SharedPrefConstants.sroCode);
...
/*       String? latitude = prefs.getString(SharedPrefConstants.latitudeKey);
      ...
      if (connection) {
        FinalSubmitPayload payload = FinalSubmitPayload(
            ...
            isSaveType: "save");
        final jsonPayload = jsonEncode(payload.toJson());
        AppLogger().logDebug("SAVED $jsonPayload");
        if (!context.mounted) return;
        FinalSubmitResponse? submitResponse =
            await VillagewiseClusterInspectionFinalSubmitRepository()
                .finalSubmitApi(context, payload);
...
// Clearing values for Upload Details
    ...
    LocalStoreHelper().removeData(SharedPrefConstants.sroCode);
    ...
    final clusterwiseApplDetailsProvider =
        Provider.of<ClusterApplicationDetailsViewModel>(context, listen: false);
    final documentDownloadProvider =
        Provider.of<DocumentDownloadViewModel>(context, listen: false);
    layoutClusterwiseApplDetailsProvider.clearAll();
    documentDownloadProvider.clearAll();
    clusterwiseApplDetailsProvider.clearAllControllers();
    //check details
    final uploadPlotDetailsProvider =
        Provider.of<UploadPlotDetailsViewModel>(context, listen: false);
    uploadPlotDetailsProvider.clearAll();

    //add coordinates
    final addCoordinatesProvider =
        Provider.of<AddCoordinatesViewModel>(context, listen: false);
    addCoordinatesProvider.clearAll();

    final checkDetailsProvider =
        Provider.of<CheckDetailsViewModel>(context, listen: false);
    checkDetailsProvider.clearAll();

    final paymentDetailsProvider =
        Provider.of<PaymentDetailsViewmodel>(context, listen: false);
    paymentDetailsProvider.clearAll();

    final recommendationsProvider =
        Provider.of<RecommendationsViewModel>(context, listen: false);
    recommendationsProvider.clearAll();

    final savedApplicationDetailsProvider =
        Provider.of<SavedApplicationDetailsViewModel>(context, listen: false);
    savedApplicationDetailsProvider.clearAll();
...
// Bulk Update Submit with Validations
  Future<void> bulkUpdateRecords(BuildContext context, String remarks) async {
    try {
      if (!kReleaseMode) debugPrint("BULK UPDATING");
      SharedPreferences prefs = await SharedPreferences.getInstance();
      //Retrieve Checklist
      String? checkListString =
          prefs.getString(SharedPrefConstants.checkListKey);
      ...
      //GetCheckList Questions
      final checklistQues = await getQuesCheckListFromSharedPreferences(
          SharedPrefConstants.questionsChecklist);
      if (!kReleaseMode) {
        debugPrint("Selected ClusterId:: ${AppConstants.clusterId}");
      }
      if (checkList.isEmpty) {
        if (!context.mounted) return;
        ValidationIoSAlert().showAlert(context,
        ...
        if (await internetCheck()) {
          if (isCheckDetailsAnswered) {
            if (selectedRecommendationValue?.recommendation == null ||
              ...
              final userName =
                  await sharedpref.readTheData(SharedPrefConstants.userName);
              final empID =
                  await sharedpref.readTheData(SharedPrefConstants.empID);
              final tokenID =
                  await sharedpref.readTheData(SharedPrefConstants.token);
              ...
              final payload = BulkUpdatePayload(
                addNotes: remarks,
                approvalFlag: selectedRecommendationValue?.recommendationValue,
                checklist: checkList,
                clusterId: AppConstants.clusterId,
                conditions: "",
                createdBy: "",
                createdIp: ip,
                designationCode: AppConstants.userType.toUpperCase(),
                empID: empID,
                ip: ip,
                isActive: "",
                roadAreaExtent: "",
