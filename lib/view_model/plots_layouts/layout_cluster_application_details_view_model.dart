...
class LayoutClusterApplicationDetailsViewModel extends ChangeNotifier {
  ...
  clearAll () {
    ...
    plotDetailsList.clear();
    selectedListMasterPlansZDP = null;
    isLoaderVisible = false;
    apiOTP = "";
    isSroEdited = false;
    totalNoOfPlotsController.clear();
    soldPlotsController.clear();
    unsoldPlotsController.clear();
          ...
          if (applicationsCount.isNotEmpty) {
            ...
            if (selectedListMasterPlansZDP == null) {
              ...
              selectedListMasterPlansZDP = listMasterPlansZDP.firstWhere(
                (element) => element.landUseId == selectedListMasterPlansZDP?.landUseId,
    ...
    if (isSroEdited == true && sroCode.length != 4) {
      ...
      if (AppConstants.isLayoutPlot == "L" &&
          ...
          unsoldPlotsController.text != "0") {
            print("entered inside 257");
        Navigator.pushNamed(context, AppRoutes.addUnsoldPlotDetails);
      } else if (AppConstants.isLayoutPlot == "L" &&
          selectedUnsoldPlotDetails == 'Y' &&
          unsoldPlotsController.text == "0") {
            print("entered inside 259");
        ValidationIoSAlert()
            .showAlert(context, description: "unable to process");
      } else {
            print("entered inside 261");
  ...
  void changeMasterPlanZDP(ListMasterPlansZDP? newValue) {
    ...
    print("Selected Master Plan ZDP: ${selectedListMasterPlansZDP?.landUseName} ${selectedListMasterPlansZDP?.landUseId}");
    notifyListeners();
  }
    ...
    if (otpText.isEmpty || otpText.length < 4) {
      ...
      SuccessCustomCupertinoAlert().showAlert(
          ...
          onPressed: () async {
            ...
            await updateMobileProvider.updateMobileApi(
              context,
