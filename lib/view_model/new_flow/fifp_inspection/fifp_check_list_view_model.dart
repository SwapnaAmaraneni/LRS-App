...
import 'package:lrsofficer/repository/new_flow/fee_paid_check_list_repository.dart';
...
class FifpCheckListViewModel with ChangeNotifier {
  ...
  Future<void> submit(BuildContext context) async {
    ...
    if (validateQues(context, applications, answeredChecklist)) {
      ...
      if (userType.toString().toLowerCase() == "tp") {
        if (AppConstants.fifpFlag == "FP") {
          Navigator.pushNamed(context, AppRoutes.feePaidPaymentDetails);
        } else if (AppConstants.fifpFlag == "FI") {
          Navigator.pushNamed(context, AppRoutes.fifpPaymentDetails);
        }
      } else {
        Navigator.pushNamed(context, AppRoutes.fifpRecommendations);
      }
    }
  ...
  Future<void> checkDetailsViewModel(BuildContext context) async {
    if (await internetCheck()) {
      ...
      if (!context.mounted) return;
      CheckDetailsResponse? response = await FifpCheckDetailsRepository()
          .fifpCheckDetailsRepo(context, request);
      responses.clear();
...
/*         responses.addAll(applications
  ...
  Future<void> getDropdownValues(BuildContext context, String quesID) async {
    if (await internetCheck()) {
      ...
      if (!context.mounted) return;
      QuestionareDropdownResponse? response = await FifpCheckDetailsRepository()
