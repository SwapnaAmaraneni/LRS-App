...
import 'package:lrsofficer/repository/new_flow/final_submit_repository.dart';
...
class FifpRecommendationsViewModel with ChangeNotifier {
    ...
    if (AppConstants.userType.toLowerCase() == "tp") {
      if ((plotNo ?? "").isEmpty || plotNo == "") {
        ...
        if (isCheckDetailsAnswered) {
          if ((AppConstants.fifpFlag == "FI") &&
              (conversionCharges == null || conversionCharges == '')) {
            if (!context.mounted) return;
          ...
          } else if ((AppConstants.fifpFlag == "FI") &&
              (isCalcutae == null || isCalcutae == false)) {
            if (!context.mounted) return;
       ...
       */
            ...
            if (connection) {
              ...
              FinalSubmitPayload payload = FinalSubmitPayload(
                  ...
                  fifpFlag: AppConstants.fifpFlag,
                  isSaveType: "submit");
              ...
              submitResponse =
                  await FifpFinalSubmit().finalSubmitApi(context, payload);
        ...
        if (selectedRecommendationValue?.recommendation == null ||
          ...
          if (connection) {
            ...
            FinalSubmitPayload payload = FinalSubmitPayload(
                ...
                isSaveType: "submit");
            final jsonPayload = jsonEncode(payload.toJson());
            AppLogger().logDebug("submit fifp payload $jsonPayload");
            if (!context.mounted) return;
            FinalSubmitResponse? submitResponse;
            submitResponse =
                await FifpFinalSubmit().finalSubmitApi(context, payload);
