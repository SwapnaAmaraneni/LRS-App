...
import 'package:lrsofficer/repository/igrs_repo/igrs_application_details_repository.dart';
...
class IGRSApplicationDetailsViewModel extends ChangeNotifier {
  ...
  Future<void> getClusterApplDetails(BuildContext context) async {
    if (await internetCheck()) {
      ...
      SavedApplicationDetailsResponse? response =
          await IGRSApplicationDetailsRepository()
  ...
  Future<void> navigateToUploadDocsScreen(
    ...
    if (plotNo.isEmpty || plotNo == "") {
      ...
      if (!context.mounted) return;
      Navigator.pushNamed(context, AppRoutes.igrsUploadPlotDetails);
    }
  }
    ...
    if (otpText.isEmpty || otpText.length < 4) {
      ...
      SuccessCustomCupertinoAlert().showAlert(
          ...
          onPressed: () async {
            ...
            await updateMobileProvider.updateIGRSMobile(
              context,
  ...
  Future<void> revenueNavigation({
    ...
    if (!context.mounted) return;
    Navigator.pushNamed(context, AppRoutes.igrsCheckList);
  }
}
